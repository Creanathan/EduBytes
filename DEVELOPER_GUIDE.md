...
- **Manual Edit & Reset**:
    - `editCell(index, field, newValue)` handles sanitization and triggers a `showToast()` visual confirmation.
    - `resetData()` provides a hard-reset by clearing `localStorage` and reloading the page, allowing for a fresh 'REIMPORT' from the source.
